...
<body>
      ...
      <div class="card-body">
        <div class="table-responsive">
          <table class="table align-middle">
            ...
            <tbody>
              <?php include_once("../../services/teachers/get_teachers.php"); ?>       
            </tbody>
          </table>
        </div>
      </div>
  ...
  <div class="modal fade" id="modalEliminar" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        ...
        <form id="formEliminar" method="POST" action="../../services/teachers/delete_teacher.php">
  ...
  <script>
    // agregar evento de carga de la pagina (para evitar apuntar a null en algun elemento)
    document.addEventListener('DOMContentLoaded', () => {

      //modal para crear /actualizar el profesor
      const modalProfesor = document.getElementById('modalProfesor');

      // modal para eliminar el profesor
      const modalEliminar = document.getElementById('modalEliminar');

      //verficar si el modal se cargo correctamente (no es null)
      if (modalEliminar) {

      //agregar evento de bootstrap para modales
        modalEliminar.addEventListener('show.bs.modal', event => {
          const boton = event.relatedTarget; //elemeto que activo el modal
          document.getElementById('delete-id').value = boton.getAttribute('data-id'); //valor del id guardado
        });
      }
      
      //ver si el modal del profesor cargo correctamente
      if (modalProfesor) {
        //evento de modales
        modalProfesor.addEventListener('show.bs.modal', event => {

        // elemento que activo el modal
          const boton = event.relatedTarget; 
          
          //formulario
          const form = document.getElementById('formProfesor');
          const titulo = document.getElementById('modalTitulo'); //tiulo del modal (dinamico)
          const btnSubmit = document.getElementById('btnSubmitModal');// boton del modal (texto dinamico)
          
          // verificar el modo del modal (si es de edicion o  para crear un profe)
          const isCreateMode = boton.getAttribute('data-mode') === 'create';

          // verificar si es el modo del modal es para crear
          if (isCreateMode) {
            
          //elementos con texto dinamico
            titulo.textContent = "Añadir Profesor";
            btnSubmit.textContent = "Agregar profesor";
            form.action = '../../services/teachers/register_teacher.php';
            form.reset(); //limpiar el formulario

            //si es modo de crear los campos del correo  y contraseña obligatorios
          ...
          } else { // si no es modo de crear es modo de actualizar
            
            //texto dinamico
          titulo.textContent = "Editar Profesor"; 

            btnSubmit.textContent = 'Guardar Cambios'; 
            form.action = "../../services/teachers/update_teacher.php"; 
            
            // en el modo de actualizacion necesitamos los datos del profesor guardados
            ...
            // los campos de contraseña y correo no son posibles de cambiarlos
